...
From 2024 to 2040, Sailor Shift released an impressive 38 songs and albums, establishing herself as a prolific artist. A pivotal moment in her career happened in 2028, when she released four songs, including one single called ***High Tide Heartbeat***, which went viral and marked her breakthrough into worldwide recognition.

The vast majority of her work (36 out of 38 releases) is rooted in the Oceanus Folk genre, reflecting the strong and consistent artistic identity for which she is known. However, she also demonstrated a willingness to experiment, with one album each in the Americana and Synthwave genres, showcasing her creative range that extends beyond her signature Oceanus Folk style.
...
The network graph above shows all persons and musical groups that has influenced Sailor Shift. Alternatively, this information is also available in the table below.

Although many individuals and musical groups have influenced Sailor Shift's work, most have done so only once, influencing one of her songs or albums.

One figure who stands out is **Wei Zhao**. whose influence appears more than once throughout her career. Wei Zhao produced *Susurros de Passion*, which inspired Sailor Shift's album *Salty Dreams*, and also composed *Silent Steps in the Forest's Embrace*, which influenced her song *Moon Over the Tide*. Other than Wei Zhao, Ivy Echos, which Sailor Shift is a member of, has also played a signifcant role in shaping her musical style. Together, they stand out as the most influential forces in her music journey.
:::
...
The network graph and accompanying table shows all artists Sailor Shift has collaborated with. In total, she has collaborated with 48 different artists, with detailed information available in the table below.

However, despite her extensive collaboration network and status as a superstar, it is worth noting that Sailor Shift has not directly or indirectly influenced other artists. None of her songs or albums have been referenced or cited as inspiration in subequent works, indicating that her music, while widely recognised by the public, has yet to leave a documented influence on her peers within the industry.
:::
...
The network graph displays all person/musical groups who have produced Oceanus Folk songs and albums.

While Sailor Shift is clearly embedded within the genre, she is connected to only a portion of the Oceanus Folk community. Most artists remain outside her influence network, appearing as distant connections (3rd to 13th degree) or entirely disconnected.

Thus, Sailor Shift's influence on the broader Oceanus Folk network can be considered moderate. She plays a visible role within a specific circle, but her impact does not span the full extent of the genre's creative ecosystem.
:::

::: callout-warning
## Other Insights

The Oceanus Folk community demonstrates a focus on collaboration rather than passive influence. Instead of simply drawing inspiration from another artist's existing works, artists actively work together to produce songs or albums.

This collaborative spirit is clearly reflected in the network graph, where 'Influenced By' connections are sparse, underscoring that creative partnerships, rather than indirect influence, drives the community's artistic growth.
:::
...
We create the bayesian surprise plot as follows:
...
The two plots for cumulative values and bayesian surprise are combined.

Codes for Cumulative Plot:
...
Both plots are combined as follows:
...
The table summarises the outward influence:
...
The Sankey for outward influence is created:
...
### 3.2.2 From which genres does Oceanus Folk draw most of its contemporary inspiration?

This analysis examines the contemporary influences on Oceanus Folk by using the genre influence patterns methodology similar to Section 3.2.1.
...
Similar to Section 3.2.1, Sankey diagram is plotted but reversed as we are looking at inward influence to Oceanus Folk.
...
Given that both plots are similar, we can combine the inward and outward influence of Oceanus Folk genre in the plot below:
...
::: callout-note
Interestingly, the Sankey diagram highlights a striking similarity in Oceanus Folk's incoming and outgoing influence patterns where the top three incoming influences (Oceanus Folk, Indie Folk, and Sythnwave) are also the top three outgoing influences, in the same order. This suggests a mutually reinforcing exchange of stylistic elements, indicating that Oceanus Folk both shapes and is shaped by a handful of genres.
:::

The table can also be combined as follows:

```{r}
#| code-fold: true
#| code-summary: "Show Code"
# Set fixed genre
selected_genre <- "Oceanus Folk"
...
genre_influence_stats <- creator_and_songs_and_influences_and_creators_collaborate %>%
  ...
  summarize(
    ...
    oceanus_influences = n_distinct(na.omit(infuence_music_collaborate[influence_genre == selected_genre])),
...
# Inward influence
genre_influenced_by_stats <- creator_and_songs_and_influenced_by_creator %>%
  filter(song_genre == selected_genre, !is.na(influenced_by_genre)) %>%
  distinct(song_to, influenced_by, influenced_by_genre) %>%
  group_by(influenced_by_genre) %>%
  summarize(
    influenced_by = n_distinct(na.omit(influenced_by)),
...
combined %>%
  rename(
    `Total Music` = Total_Music,
    `Influencing Oceanus` = Genre_Influencing_Oceanus,
    `% Oceanus (Inward)` = Perc_Oceanus_In,
    `Oceanus Influenced` = Oceanus_Influence,
    `% Oceanus (Outward)` = Perc_Oceanus
  ) %>%
  kable("html", caption = glue("Influence Table for {selected_genre}"), digits = 1, align = "c") %>%
  kable_styling(full_width = TRUE, bootstrap_options = c("striped", "hover", "condensed")) %>%
  scroll_box(width = "100%", height = "400px")
```

## 3.3 Which top artists have been most influenced by Oceanus Folk?
...
sankey_df <- creator_influenced_by_stats %>%
  ...
  target_colours <- rep("#B0B0B0", n_targets)
...
![](images/clipboard-1316413798.png)
...
Among the three artists, Sailor Shift is the most recent entrant to the music industry, debuting in 2024. She maintained a steady release schedule through 2040, ultimately producing 38 releases, surpassing Jay Walters' 37 releases in 2040. This establishes her to be the most prolific artist. By comparison, Min Fu has only released 12 records.

While Sailor Shift released more music, Jay Walters stands out as the most popular artist, with 35 hits that made the top record charts. Sailor Shift follows with 25 chart topping hits. However, it is worth noting that despite Min Fu's limited releases, all 12 of his releases made the charts.

In terms of influence, Min Fu emerges as the most impactful artist. He influenced/collaborated with 598 artists and influenced the creation of 192 musical works. Jay Walters has a much lower influence having influenced/collaborated with 51 artists and 50 music. Sailor Shift, despite her status as a superstar, has collaborated with 48 artists but has not shown any measureable influence on the broader music scene in terms of influencing musical works.

However, Sailor Shift's influence is the most recent, with new artist connections as recent as 2040. In comparison, Jay Walters' period of influence concluded earlier, with his last known connection in 2035, while Min Fu's last recorded influence dates back even further in 2030.
:::
...
### 4.3.5 Predicting who the next three Oceanus Folk stars with be over the next five years
...
-   Xia Jia follows closely behind on the Star Factor list and her influence is equally large having influenced 178 Artists and 42 Music with her Oceanus Folk music.
